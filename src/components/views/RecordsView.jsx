...
const SectionHeader = ({ title, recordCount, icon, isOpen, onClick }) => (
  ...
  >
    <div className="flex items-center justify-between">
      ...
      <FaChevronDown
        className={`text-2xl text-slate-500 transform transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
      />
    </div>
  </div>
);
...
const RecordsView = ({
  ...
  deleteRecord = () =>
    console.error(
      "CRASH PREVENTED: The 'deleteRecord' function was not passed to the RecordsView component.",
    ),
}) => {
  const [openSection, setOpenSection] = useState("sounds");

  const soundRecords = [...(records?.sounds || [])].sort(
    (a, b) =>
      (b.timestamp?.seconds || b.timestamp) -
      (a.timestamp?.seconds || a.timestamp),
  );
  const varnmalaRecords = [...(records?.varnmala || [])].sort(
    (a, b) =>
      (b.timestamp?.seconds || b.timestamp) -
      (a.timestamp?.seconds || a.timestamp),
  );
  const storyRecords = [...(records?.stories || [])].sort(
    (a, b) =>
      (b.timestamp?.seconds || b.timestamp) -
      (a.timestamp?.seconds || a.timestamp),
  );
  ...
  const sections = [
    {
      ...
      renderRow: (rec) => (
        <tr key={rec.id}>
          ...
          <td className="p-4">
            <button
              onClick={() => deleteRecord(rec.id, "sounds")}
              aria-label={`Delete record for ${rec.sound}`}
    ...
    },
    {
      ...
      renderRow: (rec) => (
        <tr key={rec.id}>
          ...
          <td className="p-4">
            <button
              onClick={() => deleteRecord(rec.id, "varnmala")}
              aria-label={`Delete record from ${formatSafeDate(rec.timestamp)}`}
              className="text-slate-400 hover:text-red-500 transition-colors"
            >
              <FaTrash />
            </button>
    ...
    },
    {
      ...
      renderRow: (rec) => (
        <tr key={rec.id}>
          ...
          <td className="p-4">
            <button
              onClick={() => deleteRecord(rec.id, "stories")}
              aria-label={`Delete record for ${rec.storyTitle}`}
              className="text-slate-400 hover:text-red-500 transition-colors"
            >
              <FaTrash />
            </button>
                      ...
                      <thead className="bg-slate-100 dark:bg-slate-700">
                        <tr>
                          {section.headers.map((header, index) => (
                            <th
                              key={header}
                              className={`p-4 ${index === 0 ? "rounded-tl-lg" : ""} ${index === section.headers.length - 1 ? "rounded-tr-lg" : ""}`}
                            >
                              {header}
                            </th>
                          ))}
