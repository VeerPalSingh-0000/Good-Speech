...
const HistoryView = ({ records }) => {
  ...
  const { soundRecordsByDate, sortedDates } = useMemo(() => {
    ...
    (records.sounds || []).forEach(rec => {
        try {
            const date = rec.timestamp?.seconds ? new Date(rec.timestamp.seconds * 1000) : new Date(rec.timestamp);
        ...
        } catch (e) { /* Ignore records with invalid timestamps */ }
    });

    const finalPivotedData = {};
    for (const dateKey in groupedByStandardDate) {
        // Sort records for the day by timestamp, oldest first
        const dayRecords = groupedByStandardDate[dateKey].sort((a, b) => 
            (a.timestamp?.seconds || a.timestamp) - (b.timestamp?.seconds || b.timestamp)
        );
    ...
    Object.keys(groupedByStandardDate).forEach(dateKey => {
        const displayDate = formatSafeDate(groupedByStandardDate[dateKey][0].timestamp);
        dateSortMap[displayDate] = new Date(dateKey).getTime();
    });

    const sortedDisplayDates = Object.keys(finalPivotedData).sort((a, b) => dateSortMap[b] - dateSortMap[a]);

    return { soundRecordsByDate: finalPivotedData, sortedDates: sortedDisplayDates };
  }, [records.sounds]);
  ...
  const SectionHeader = ({ title, recordCount, sectionKey, icon }) => (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <i className={`fas fa-chevron-down text-2xl text-slate-500 transform transition-transform duration-300 ${openSection === sectionKey ? 'rotate-180' : ''}`}></i>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700"><h4 className="font-bold text-lg">कुल सत्र</h4><p className="text-3xl font-bold text-blue-500">{stats.totalSessions}</p></div>
        <div className="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700"><h4 className="font-bold text-lg">कुल समय</h4><p className="text-3xl font-bold font-mono text-green-500">{stats.totalTime}</p></div>
        <div className="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700"><h4 className="font-bold text-lg">सर्वश्रेष्ठ स्वर समय</h4><p className="text-3xl font-bold font-mono text-purple-500">{stats.bestSound}</p></div>
      </div>

      {/* Sound Records Section */}
      <div className="space-y-4">
        <SectionHeader title="स्वर अभ्यास रिकॉर्ड" recordCount={records.sounds?.length || 0} sectionKey="sounds" icon="fas fa-microphone" />
        <AnimatePresence>
        {openSection === 'sounds' && (
          <motion.div initial={{height: 0, opacity: 0}} animate={{height: 'auto', opacity: 1}} exit={{height: 0, opacity: 0}} className="overflow-hidden">
            <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-inner space-y-6">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {Object.keys(soundRecordsByDate[date]).map(session => {
                          const sessionData = soundRecordsByDate[date][session];
                          const hasNewBest = Object.values(sessionData).some(rec => rec.isNewBest);
                          return (
                            <tr key={session}>
                              <td className="p-4 font-bold">{session}</td>
                              {['आ', 'ई', 'ऊ'].map(sound => (
                                <td key={sound} className="p-4 text-center font-mono">
                                  {sessionData[sound] ? (
                                    <span className={sessionData[sound].isNewBest ? 'text-yellow-500 font-bold' : ''}>
              ...
              {sortedDates.length === 0 && <p className="p-8 text-center text-slate-500">No sound records yet.</p>}
            </div>
          </motion.div>
        )}
        </AnimatePresence>
      ...
      {/* Varnmala and Stories sections */}
      <div className="space-y-4">
        <SectionHeader title="वर्णमाला अभ्यास रिकॉर्ड" recordCount={records.varnmala?.length || 0} sectionKey="varnmala" icon="fas fa-list" />
        {openSection === 'varnmala' && (
           <motion.div initial={{height: 0, opacity: 0}} animate={{height: 'auto', opacity: 1}} exit={{height: 0, opacity: 0}} className="overflow-hidden">
            <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-inner">
              <table className="w-full text-left">
                <thead className="bg-slate-100 dark:bg-slate-700"><tr><th className="p-4 rounded-tl-lg">समय</th><th className="p-4">गुणवत्ता</th><th className="p-4 rounded-tr-lg">दिनांक</th></tr></thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
      ...
      <div className="space-y-4">
        <SectionHeader title="पठन अभ्यास रिकॉर्ड" recordCount={records.stories?.length || 0} sectionKey="stories" icon="fas fa-book" />
        {openSection === 'stories' && (
           <motion.div initial={{height: 0, opacity: 0}} animate={{height: 'auto', opacity: 1}} exit={{height: 0, opacity: 0}} className="overflow-hidden">
            <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-inner">
              <table className="w-full text-left">
                 <thead className="bg-slate-100 dark:bg-slate-700"><tr><th className="p-4 rounded-tl-lg">कहानी</th><th className="p-4">समय</th><th className="p-4">प्रतिशत</th><th className="p-4 rounded-tr-lg">दिनांक</th></tr></thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
